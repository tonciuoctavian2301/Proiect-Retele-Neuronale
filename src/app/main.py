...
# Încercăm să importăm winsound (doar pentru Windows)
try:
    import winsound
except ImportError:
    winsound = None
...
class DrowsinessApp:
    def __init__(self, window, window_title):
        ...
        self.alarm_on = False  # Variabilă pentru a evita suprapunerea sunetelor
    ...
    def play_alarm_sound(self):
        """Rulează alarma pe un fir separat."""
        try:
            # Încercare 1: Windows Beep
            if winsound:
                winsound.Beep(2500, 1000)

            # Încercare 2: System Bell (Backup)
            print('\a')
        except:
            pass
        ...
        if self.is_running:
            ...
            if ret:
                ...
                for (x, y, w, h) in faces:
                    ...
                    # SENSIBILITATE OCHI (minNeighbors=3)
                    eyes = self.eye_cascade.detectMultiScale(upper_face, scaleFactor=1.1, minNeighbors=3)

                    # FALLBACK: Lipsă ochi = Somn
                    ...
                    mx = max(0, mx)
                    my = max(0, my)
                    if mx + mw > frame.shape[1]: mw = frame.shape[1] - mx
                    if my + mh > frame.shape[0]: mh = frame.shape[0] - my
                    ...
                    try:
                        if mw > 0 and mh > 0:
                            ...
                            if input_mouth is not None:
                                ...
                                if pred_m > 0.4:
                                    cv2.putText(frame, "CASCAT!", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255),
                                                2)
                                    self.score += 2
                ...
                if eyes_closed_in_frame:
                    self.score += 3  # Creștere RAPIDĂ
                    self.lbl_stare.config(text="STARE: OCHI ÎNCHIȘI", fg="red")
                else:
                    self.score -= 1  # Scădere LENTĂ
                ...
                # --- DECLANȘARE ALARMĂ (> 40) ---
                if self.score > 40:
                    cv2.putText(frame, "WAKE UP!", (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 2.5, (0, 0, 255), 5)
                    cv2.rectangle(frame, (0, 0), (frame.shape[1], frame.shape[0]), (0, 0, 255), 20)

                    if not self.alarm_on:
                        self.alarm_on = True
                        t = threading.Thread(target=self.play_alarm_sound)
                        t.daemon = True
                        t.start()

                elif self.score > 20:
                    cv2.putText(frame, "ATENTIE!", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 165, 255), 4)
...
if __name__ == "__main__":
    root = tk.Tk()
    app = DrowsinessApp(root, "Sistem Monitorizare - WAKE UP MODE")
    root.mainloop()